...
fn every_menu_screen_builds_centered_game_buttons() {
    ...
    for (screen, expected) in [
        ...
        assert!(layout.buttons.iter().all(|button| button.bounds.x > 0
            && button.bounds.y > 0
            && button.bounds.x + button.bounds.width as i32 <= canvas.width as i32
            && button.bounds.y + button.bounds.height as i32 <= canvas.height as i32));
    }
}
...
fn hit_testing_converts_physical_window_pixels_to_logical_space() {
    ...
    assert_eq!(
        layout.hit_test(canvas, logical_x * 2.0, logical_y * 2.0, (1920, 1080)),
        Some((0, START))
    );
    assert_eq!(layout.hit_test(canvas, 0.0, 0.0, (1920, 1080)), None);
}
